...
l = 3 # Seillänge
h = 1 # Schrittweite
c = np.random.uniform(0.92, 1) # Dämpfungskonstante
time = np.linspace(0, 100, 1000)

# Zufällige startauslenkung/-Geschwindigkeit
init_alpha = 0
init_alphaDeriv = np.random.uniform(50)
...
def step(t):
    ...
    pos.set_data([np.cos(radians(alpha[t] - 90)) * l], 
                 [np.sin(radians(alpha[t] - 90)) * l])
    text.set_text(str(t))
    lin.set_data([originPoint[0], np.cos(radians(alpha[t] - 90)) * l], 
                 [originPoint[1], np.sin(radians(alpha[t] - 90)) * l])
